...
class ConfigurationRepositoryIT extends BaseDataExportInitializerIT {
  ...
  void saveConfigurationTest() {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      configurationRepository.save(ConfigurationEntity.builder().key("config1").value("1").build());
      configurationRepository.save(ConfigurationEntity.builder().key("config2").value("2").build());
      configurationRepository.save(ConfigurationEntity.builder().key("config3").value("3").build());
      ...
      // Update config3.
      configurationRepository.save(ConfigurationEntity.builder().key("config3").value("4").build());
  ...
  void getConfigurationTest() {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      // Update config2.
      configurationRepository.save(ConfigurationEntity.builder().key("config2").value("3").build());
      expectedValue = "3";
      actualValue = configurationRepository.getReferenceById("config2").getValue();
      assertEquals(expectedValue, actualValue);
    }
