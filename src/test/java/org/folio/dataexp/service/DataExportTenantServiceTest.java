...
class DataExportTenantServiceTest {
  ...
  void testCreateOrUpdateTenantShouldInitializeDefaultConfigurations() {
    // TestMate-55e7ca0e41910724881d2059e6388413
    // Given
    var tenantAttributes = new TenantAttributes();
    var tenantId = "test_tenant";
    var inventoryConfig = new Config().key("inventory_record_link").value("http://localhost/inventory/view/");
    var sliceSizeConfig = new Config().key(SlicerProcessor.SLICE_SIZE_KEY).value(String.valueOf(SlicerProcessor.DEFAULT_SLICE_SIZE));
    
    when(context.getTenantId()).thenReturn(tenantId);
    when(context.getFolioModuleMetadata()).thenReturn(folioModuleMetadata);
    when(configurationService.produceInventoryRecordLinkBasedOnTenantBaseUrl()).thenReturn(inventoryConfig);
    when(configurationService.upsertConfiguration(any(Config.class))).thenReturn(inventoryConfig, sliceSizeConfig);
    ...
    assertThat(capturedConfigs).extracting(Config::getKey)
        .containsExactlyInAnyOrder("inventory_record_link", SlicerProcessor.SLICE_SIZE_KEY);
    assertThat(capturedConfigs).extracting(Config::getValue)
