...
class PermissionsValidatorTest {
  ...
  private static final String UNLOCK_PERMISSION = "data-export.job-profile.unlock";
  ...
  // ========== Tests for checkUnlockJobProfilePermission ==========

  @Test
  void shouldReturnTrue_whenUserHasUnlockPermission() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(UNLOCK_PERMISSION);
    when(permissionsProvider.getUserPermissions(TENANT_ID, userId.toString()))
        .thenReturn(List.of(UNLOCK_PERMISSION));

    // When
    boolean result = permissionsValidator.checkUnlockJobProfilePermission();

    // Then
    assertThat(result).isTrue();
    verify(requiredPermissionResolver).getUnlockJobProfilePermission();
    verify(permissionsProvider).getUserPermissions(TENANT_ID, userId.toString());
  }

  @Test
  void shouldReturnFalse_whenUserDoesNotHaveUnlockPermission() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(UNLOCK_PERMISSION);
  ...
  @Test
  void shouldReturnFalse_whenUserHasEmptyPermissionsList_forUnlock() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(UNLOCK_PERMISSION);
  ...
  @Test
  void shouldReturnTrue_whenUserHasMultiplePermissions_includingUnlock() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(UNLOCK_PERMISSION);
    when(permissionsProvider.getUserPermissions(TENANT_ID, userId.toString()))
        .thenReturn(List.of("permission1", UNLOCK_PERMISSION, "permission2"));
  ...
  @Test
  void shouldUseCorrectTenantId_whenCheckingUnlockPermission() {
    // Given
    String customTenantId = "custom-tenant";
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(customTenantId);
  ...
  @Test
  void shouldReturnFalse_whenUnlockPermissionIsNull() {
    // Given
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(null);
  ...
  @Test
  void shouldReturnFalse_whenUserHasSimilarButNotExactUnlockPermission() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(UNLOCK_PERMISSION);
    when(permissionsProvider.getUserPermissions(TENANT_ID, userId.toString()))
        .thenReturn(
            List.of("data-export.job-profile.unlock.extra", "data-export.job-profile"));
  ...
  @Test
  void shouldReturnTrue_whenUserHasBothLockAndUnlockPermissions() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getLockJobProfilePermission()).thenReturn(LOCK_PERMISSION);
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(UNLOCK_PERMISSION);
    when(permissionsProvider.getUserPermissions(TENANT_ID, userId.toString()))
        .thenReturn(List.of(LOCK_PERMISSION, UNLOCK_PERMISSION));

    // When
    boolean lockResult = permissionsValidator.checkLockJobProfilePermission();
    boolean unlockResult = permissionsValidator.checkUnlockJobProfilePermission();
  ...
  @Test
  void shouldReturnFalse_whenUserHasOnlyLockPermission_butNotUnlock() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getUnlockJobProfilePermission()).thenReturn(UNLOCK_PERMISSION);
  ...
  @Test
  void shouldReturnFalse_whenUserHasOnlyUnlockPermission_butNotLock() {
    // Given
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn(TENANT_ID);
    when(requiredPermissionResolver.getLockJobProfilePermission()).thenReturn(LOCK_PERMISSION);
