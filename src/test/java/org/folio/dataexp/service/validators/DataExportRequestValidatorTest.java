...
private DataExportRequestValidator dataExportRequestValidator;
  ...
  return Stream.of(
      Arguments.of(ExportRequest.IdTypeEnum.INSTANCE, FileDefinition.UploadFormatEnum.CSV, randomMappingProfileId, randomJobProfileId),
      Arguments.of(ExportRequest.IdTypeEnum.HOLDING, FileDefinition.UploadFormatEnum.CSV, randomMappingProfileId, randomJobProfileId),
      Arguments.of(ExportRequest.IdTypeEnum.AUTHORITY, FileDefinition.UploadFormatEnum.CSV, DEFAULT_AUTTHORITY_PROFILE_ID, randomJobProfileId),
      Arguments.of(ExportRequest.IdTypeEnum.AUTHORITY, FileDefinition.UploadFormatEnum.CSV, DEFAULT_AUTTHORITY_PROFILE_ID, deletedJobProfileId)
  );
}
...
void validateAuthorityExportWithNonDefaultMappingProfileShouldThrowException() {
  // TestMate-2fc8251d2068a6fd01c8f4df44bbb2ce
  // Given
  ...
  var exception = assertThrows(DataExportRequestValidationException.class,
      () -> dataExportRequestValidator.validate(exportRequest, fileDefinition, nonDefaultMappingProfileId));
  // Then
  assertEquals("For exporting authority records only the default authority job profile is supported", exception.getMessage());
...
void validateShouldPassWhenRequestIsValid(ExportRequest.IdTypeEnum idType, FileDefinition.UploadFormatEnum uploadFormat, String mappingProfileId, UUID jobProfileId) {
  // TestMate-a8abdaa6265cad857cf8d0b07c4dab5d
  // Given
  var fileDefinition = new FileDefinition()
  ...
  // When & Then
  assertDoesNotThrow(() -> dataExportRequestValidator.validate(exportRequest, fileDefinition, mappingProfileId));
