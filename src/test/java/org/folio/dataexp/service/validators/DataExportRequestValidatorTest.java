...
class DataExportRequestValidatorTest {
    ...
    private static final UUID DELETED_RECORDS_JOB_PROFILE_ID = UUID.fromString("2c9be114-6d35-4408-adac-9ead35f51a27");
  ...
  void validateShouldPassWhenIdTypeIsHoldingAndUploadFormatIsCsv() {
    ...
    var validator = new DataExportRequestValidator(errorLogService);
    // When & Then
    assertDoesNotThrow(() -> validator.validate(exportRequest, fileDefinition, UUID.randomUUID().toString()));
  ...
  void validateShouldPassWhenIdTypeIsAuthorityAndMappingProfileIsDefault() {
    ...
    var validator = new DataExportRequestValidator(errorLogService);
    // When & Then
    assertDoesNotThrow(() -> validator.validate(exportRequest, fileDefinition, DEFAULT_AUTTHORITY_PROFILE_ID));
    verifyNoInteractions(errorLogService);
  }

    @ParameterizedTest
  @EnumSource(value = ExportRequest.IdTypeEnum.class, names = {"AUTHORITY"}, mode = EnumSource.Mode.EXCLUDE)
  void validateShouldThrowExceptionWhenJobProfileIsForDeletedRecordsAndIdTypeIsNotAuthority(ExportRequest.IdTypeEnum idType) {
