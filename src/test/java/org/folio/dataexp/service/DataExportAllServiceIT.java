...
class DataExportAllServiceIT extends BaseServiceInitializerIT {
  ...
  @Autowired private DataExportAllService dataExportAllService;
      ...
      await()
          ...
          .untilAsserted(
              () -> {
                ...
                var fileDefinition =
                    fileDefinitionEntityRepository
                        .getFileDefinitionByJobExecutionId(jobExecution.getId().toString())
                        .get(0)
                        .getFileDefinition();
                ...
                var fileToExport =
                    String.format(
                        "%s-%s.mrc", "instance-all", jobExecution.getJobExecution().getHrId());
  ...
  void exportAllInstancesNoErrorsCustomInstanceProfileTest() {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      var exportAllRequest =
          new ExportAllRequest()
  ...
  void exportAllInstancesNonDeletedNoErrorsTest() {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      handleReferenceData();
      var exportAllRequest =
          new ExportAllRequest().suppressedFromDiscovery(true).deletedRecords(false);
      dataExportAllService.postDataExportAll(exportAllRequest);
  ...
  void exportAllInstancesNonSuppressedNonDeletedNoErrorsTest() {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      handleReferenceData();
      var exportAllRequest =
          new ExportAllRequest().suppressedFromDiscovery(false).deletedRecords(false);
      dataExportAllService.postDataExportAll(exportAllRequest);
  ...
  void exportAllHoldingsNoErrorsTest() {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      var exportAllRequest =
          new ExportAllRequest()
              .idType(ExportAllRequest.IdTypeEnum.HOLDING)
      ...
      await()
          ...
          .untilAsserted(
              () -> {
                ...
                var fileToExport =
                    String.format(
                        "%s-%s.mrc", "holding-all", jobExecution.getJobExecution().getHrId());
                var s3path = getPathToStoredFiles(jobExecution.getId(), fileToExport);
                String outputMrcFile = Files.read(s3Client.read(s3path), Charset.defaultCharset());
  ...
  void exportAllAuthorityNoErrorsIgnoreSuppressedTest(boolean suppressedFromDiscovery) {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      var exportAllRequest =
          new ExportAllRequest()
              .idType(ExportAllRequest.IdTypeEnum.AUTHORITY)
      ...
      await()
          ...
          .untilAsserted(
              () -> {
                ...
                var fileToExport =
                    String.format(
                        "%s-%s.mrc", "authority-all", jobExecution.getJobExecution().getHrId());
                var s3path = getPathToStoredFiles(jobExecution.getId(), fileToExport);
                String outputMrcFile = Files.read(s3Client.read(s3path), Charset.defaultCharset());
  ...
  private void createCustomInstanceJobProfile() {
    var customInstanceMappingProfile =
        new MappingProfile()
            .id(CUSTOM_INSTANCE_MAPPING_PROFILE_ID)
            .name("Custom Instance Mapping Profile")
            ._default(false)
            .recordTypes(List.of(RecordTypes.INSTANCE));
    ...
    var customInstanceJobProfile =
        new JobProfile()
            .id(CUSTOM_INSTANCE_JOB_PROFILE_ID)
            .name("Custom Instance Job Profile")
            ._default(false)
            .mappingProfileId(CUSTOM_INSTANCE_MAPPING_PROFILE_ID);
    jobProfileEntityRepository.save(
        new JobProfileEntity()
            .withJobProfile(customInstanceJobProfile)
            .withId(customInstanceJobProfile.getId())
            .withName(customInstanceJobProfile.getName())
            .withMappingProfileId(CUSTOM_INSTANCE_MAPPING_PROFILE_ID));
  }

  private void createCustomHoldingsJobProfile() {
    var customHoldingsMappingProfile =
        new MappingProfile()
            .id(CUSTOM_HOLDINGS_MAPPING_PROFILE_ID)
            .name("Custom Holdings Mapping Profile")
            ._default(false)
            .recordTypes(List.of(org.folio.dataexp.domain.dto.RecordTypes.HOLDINGS));
    mappingProfileEntityRepository.save(
        new MappingProfileEntity()
    ...
    var customHoldingsJobProfile =
        new JobProfile()
            .id(CUSTOM_HOLDINGS_JOB_PROFILE_ID)
            .name("Custom Holdings Job Profile")
            ._default(false)
            .mappingProfileId(CUSTOM_HOLDINGS_MAPPING_PROFILE_ID);
    jobProfileEntityRepository.save(
        new JobProfileEntity()
            .withJobProfile(customHoldingsJobProfile)
            .withId(customHoldingsJobProfile.getId())
            .withName(customHoldingsJobProfile.getName())
