...
class LogsControllerTest {
  ...
  void getErrorLogsByQueryWhenQueryIsNotEmptyShouldUseProvidedQuery() {
    // TestMate-66a7bccb716533ddb8e51a25916a2874
    // Given
    ...
    var errorLogEntity = ErrorLogEntity.builder()
        .id(errorLogId)
        .errorLog(errorLog)
        .build();
    var page = new PageImpl<>(List.of(errorLogEntity));
    when(errorLogEntityCqlRepository.findByCql(eq(query), eq(offsetRequest))).thenReturn(page);
    // When
    ResponseEntity<ErrorLogCollection> response = logsController.getErrorLogsByQuery(query, offset, limit);
    // Then
  ...
  void getErrorLogsByQueryWhenQueryIsEmptyShouldUseAllRecordsQuery(String query) {
    // TestMate-e91d01caaab58528218a3f091ae95c07
    // Given
    var offset = 0;
    var limit = 20;
    ...
    var errorLogEntity = ErrorLogEntity.builder()
        ...
        .build();
    var page = new PageImpl<>(List.of(errorLogEntity));
    when(errorLogEntityCqlRepository.findByCql(eq(QUERY_CQL_ALL_RECORDS), eq(offsetRequest))).thenReturn(page);
    // When
    ResponseEntity<ErrorLogCollection> response = logsController.getErrorLogsByQuery(query, offset, limit);
    // Then
  ...
  void getErrorLogsByQueryShouldCorrectlyHandlePaginationParameters(int offset, int limit) {
    // TestMate-ad18ad2d00c0661267badbfe7d9e7264
    // Given
    ...
    when(errorLogEntityCqlRepository.findByCql(eq(query), any(OffsetRequest.class))).thenReturn(page);
  ...
  void getErrorLogsByQueryWhenRepositoryReturnsEmptyPageShouldReturnEmptyCollection() {
    // TestMate-2317fb6f7791c13a56ce63358500986f
    // Given
