...
public interface FolioHoldingsAllRepository extends Repository<HoldingsRecordEntity, UUID> {
   ...
   */
  @Query(
      value = "SELECT * FROM v_folio_holdings_all_non_deleted WHERE id BETWEEN ?1 AND ?2"
      + " ORDER BY id ASC",
      nativeQuery = true
  )
  Slice<HoldingsRecordEntity> findFolioHoldingsAllNonDeleted(
      UUID fromId,
      UUID toId,
      Pageable page
  );
   ...
   */
  @Query(
      value = "SELECT * FROM v_folio_holdings_all_non_deleted_non_suppressed"
   ...
   */
  @Query(
      value = "SELECT * FROM v_holdings_all_folio_deleted ORDER BY id ASC",
      nativeQuery = true
  )
   ...
   */
  @Query(
      value = "SELECT * FROM v_marc_holdings_all_non_deleted_custom_profile"
      + " WHERE id BETWEEN ?1 AND ?2"
      + " ORDER BY id ASC",
      nativeQuery = true
  )
   ...
   */
  @Query(
      value = "SELECT * FROM v_marc_holdings_all_non_deleted_non_suppressed_custom_profile"
   ...
   */
  @Query(
      value = "SELECT * FROM v_holdings_all_marc_deleted_not_suppressed_custom_profile "
      + "ORDER BY id ASC",
      nativeQuery = true
  )
