...
public interface InstanceEntityRepository extends Repository<InstanceEntity, UUID> {
  ...
  @Query(value = "SELECT * FROM v_instance_all WHERE id BETWEEN ?1 AND ?2 ORDER BY id ASC", nativeQuery = true)
  Slice<InstanceEntity> findByIdGreaterThanEqualAndIdLessThanEqualOrderByIdAsc(UUID fromId, UUID toId, Pageable page);
  @Query(value = "SELECT * FROM v_instance_all WHERE id BETWEEN ?1 AND ?2 AND id NOT IN ?3 ORDER BY id ASC", nativeQuery = true)
  Slice<InstanceEntity> findByIdGreaterThanEqualAndIdLessThanEqualAndIdNotInOrderByIdAsc(UUID fromId, UUID toId, Set<UUID> deletedMarcIds, Pageable page);
