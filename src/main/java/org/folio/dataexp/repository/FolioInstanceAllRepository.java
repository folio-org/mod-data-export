...
public interface FolioInstanceAllRepository extends Repository<InstanceEntity, UUID> {
  ...
  Slice<InstanceEntity> findFolioInstanceAll(
      UUID fromId,
      UUID toId,
      Pageable page
  );
   ...
   */
  @Query(
      value = "SELECT * FROM v_folio_all_non_deleted_suppressed WHERE id BETWEEN ?1 AND ?2"
      + " ORDER BY id ASC",
      nativeQuery = true
  )
   ...
   */
  @Query(
      value = "SELECT * FROM v_folio_instance_all_non_deleted_non_suppressed WHERE id"
      + " BETWEEN ?1 AND ?2"
      + " ORDER BY id ASC",
      nativeQuery = true
  )
   ...
   */
  @Query(
      value = "SELECT * FROM v_marc_instance_all_non_deleted_custom_profile WHERE id"
      + " BETWEEN ?1 AND ?2"
      + " ORDER BY id ASC",
      nativeQuery = true
  )
   ...
   */
  @Query(
      value = "SELECT * FROM v_instance_all_marc_deleted_not_suppressed_custom_instance_profile"
      + " ORDER BY id ASC",
      nativeQuery = true
  )
