...
public class HoldingsExportAllStrategy extends HoldingsExportStrategy {
  ...
  private void processFolioSlices(
    ...
    processFolioHoldings(
        ...
        localStorageWriter);
    log.debug("Slice size for holdings export all folio: {}", folioSlice.getContent().size());
  ...
  private void processMarcSlices(
    ...
    if (Boolean.TRUE.equals(mappingProfile.getDefault())) {
      ...
      processMarcHoldings(
          ...
          localStorageWriter);
      log.debug("Slice size for holdings export all srs marc: {}", marcSlice.getContent().size());
  ...
  private void processMarcHoldingsSlices(
    ...
    processFolioHoldings(
        ...
        localStorageWriter);
    log.debug(
        "Slice size for holdings export all marc holdings: {}",
        marcHoldingsSlice.getContent().size());
