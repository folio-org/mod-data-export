...
CREATE OR REPLACE PROCEDURE slice_instances_all_ids(jobExecutionId text, fileLocation text, sliceSize int)
LANGUAGE plpgsql
AS $$
BEGIN

 WITH RankedRows as (select rec.id id,(ROW_NUMBER() OVER (ORDER BY rec.id)) - 1 row_num
    from ${myuniversity}_mod_inventory_storage.instance rec order by id),
