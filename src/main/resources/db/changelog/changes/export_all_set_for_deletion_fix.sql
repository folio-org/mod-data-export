...
AS SELECT inst.id, jsonb FROM ${myuniversity}_mod_data_export.v_instance inst
   WHERE jsonb ->> 'source' = 'FOLIO' AND
       ((jsonb ->> 'discoverySuppress' IS NULL OR jsonb ->> 'discoverySuppress' = 'false') OR
        ((jsonb ->> 'discoverySuppress' = 'true' ) AND (jsonb ->> 'staffSuppress' IS NULL OR jsonb ->> 'staffSuppress' = 'false')));
